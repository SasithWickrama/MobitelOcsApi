            ...
            if conn['status'] == "error":                
                ...
                if self is not None:
                    ...
                    if row_count == 0:
                        ...
                        return data[0]['SATT_DEFAULTVALUE']
        except Exception as e:
            #result['data']= {"status": "error","errors": "DB Connection Error " + str(e)}
            #logger.info("Exception : %s" % ref + " - " + str(e))
...
class Pcrf:
    ...
    def pcrfAddonCreate(data, ref ):
        ...
        try:
            ...
            logger.error("Return@5 : %s" % ref + " - " + str(responsedata))
            return responsedata

        try:
            getpkg = getPackage(data['offeringID'])
            getpkgall = getPackageAll(data['offeringID']) 
            ...
            if getpkg is not None:
                ...
                if response.status_code == 200:
                    ...
                    logger.info("Return@0 : %s" % ref + " - " + str(responsedata))

                    #====SMS====
                    msg = "You have successfully activated the "+ str(getpkgall) + ", valid for 30 days. To check data balance, use the MySLT App or visit https://myslt.slt.lk/"
                    #getmobile =getMobile(data['orderObjectKey'])
                    getmobile = getNumberNew.clarityDbnew('94'+data['orderObjectKey'][-9:])
                    if getmobile == 'No data found':
                        logger.info("mobile : %s" % ref + " - " + getmobile)
                    else:
                        logger.info("mobile : %s" % ref + " - " + getmobile)
                        smsdata = {"tpno": getmobile, "msg": msg}
                        if getmobile[0:2] == '07' and len(getmobile) == 10:
                            Sendsms.sendSms(smsdata, ref)
                            return {'result': 'success', 'description': 'Recharge SMS sent Successfully', 'data': data}
                        else:
                            return {'result': 'error', 'description': 'Invalid mobile number', 'data': data}
                else:
                    responsedata = {"result": "error", "description": ref+" - "+resmsg['message'], 'data': data}
                    logger.info("Return@1 : %s" % ref + " - " + str(responsedata))
            else:
                responsedata = {"result": "error", "description": ref+" - invalid offer id", 'data': data}
                logger.info("Return@2 : %s" % ref + " - " + str(responsedata))

        except Exception as e:
            responsedata = {"result": "error", "description": ref+" - "+str(e), 'data': data}
            logger.info("Return@3 : %s" % ref + " - " + str(responsedata))
    ...
    def pcrfAddonDelete(data,ref):
        ...
        try:
                        ...
                        if(i['freeUnitTypeName'] == unitType):
                            ...
                            for item in i['itemDetail']:
                                if str(item['currentAmount']) > '0':
                                    curValue = False
                                    return {'result': 'error', 'description': "currentAmount is not null for " + unitType, 'data': unitType}
                                    
                            if curValue:
                                logger.info("curValue : %s" % ref + " - " + str(curValue))
                                            ...
                                            if val['subscriptionid'] == 'P_VB_Q_OM_PREPAID_30GB' or 'P_VB_PRE_SMYT':
                                                ...
                                                if str(val['timestamp']) is not None:
                                                    ...
                                                    if getmobile == 'No data found':
                                                        ...
                                                        if getmobile[0:2] == '07' and len(getmobile) == 10:
                                                            Sendsms.sendSms(smsdata, ref)
                                                            return {'result': 'success', 'description': 'SLT_PCRF is provisioned ', 'data': data}
                                                        else:
                                                            return {'result': 'error', 'description': 'Invalid mobile number', 'data': data}    
                                                else:
                                                    return {'result': 'error', 'description': 'Invalid Request', 'data': data}
                                                    logger.info("Return : %s" % ref + " - " + str(responsedata))  
                                return {'result': 'error', 'description': resultPcrfVas['result'], 'data': unitType}
                            return {'result': 'error', 'description': curValue, 'data': unitType}
            ...
            responsedata = {"result": "error", "msg": ref +" - Connection Issue" + " - " + e}
    ...
    def chnageSubStatus(data,ref):
        ...
        try:
            ...
            responsedata = {"result": "error", "description": ref+" - "+str(e), 'data': data}
            logger.info("Exception : %s" % ref + " - " + str(responsedata))
            ...
            if rcode == '0':
                ...
                for i in resmsg['QueryCustomerInfoResult']['Subscriber']['LifeCycleDetail']['LifeCycleStatus']:
                    if i['StatusIndex'] == curIndex:
                        ...
                        #====SMS====
                msg = "Dear Customer, the validity of connection 94"+data['orderObjectKey'][1:10]+" (registered under your ID) will expire on "+str(datetime_obj)+". Reload to continue services. Visit https://myslt.slt.lk/ "
                #getmobile =getMobile(data['orderObjectKey'])
                getmobile = getNumberNew.clarityDbnew('94'+data['orderObjectKey'][-9:])
                if getmobile == 'No data found':
                    logger.info("mobile : %s" % ref + " - " + getmobile)
                    ...
                    if getmobile[0:2] == '07' and len(getmobile) == 10:
                        Sendsms.sendSms(smsdata, ref)
                        return {'result': 'success', 'description': 'Chanage Sub Status SMS sent Successfully', 'data': data}
                    else:
                        return {'result': 'error', 'description': 'Invalid mobile number', 'data': data}
            else:
                return {'result': 'error', 'description': resmsg, 'data': data}
        ...
        if rcode == '0':
            ...
            for i in resmsg['QueryCustomerInfoResult']['Subscriber']['LifeCycleDetail']['LifeCycleStatus']:
                if i['StatusIndex'] == curIndex:
                    ...
                    #====SMS====
                    try:
                        rechargeAmount = round(float(data['rechargeAmount'])/100 , 2)
                        balanceAfterRecharge = round(float(data['balanceAfterRecharge'])/100  , 2)
                        ...
                        logger.info("orderObjectKey : %s" % ref + " - " + data['orderObjectKey'])

                        getmobile = getNumberNew.clarityDbnew('94'+data['orderObjectKey'][-9:])
                        #getmobile = getNumber.clarityDb('0372290177')#0113493122
                        logger.info("mobile : %s" % ref + " - " + str(getmobile))
                        if getmobile == 'No data found':
                            logger.info("mobile no data: %s" % ref + " - " + getmobile)
                            return {'result': 'error', 'description': 'Invalid mobile number', 'data': data}
                        else:
                            logger.info("mobile : %s" % ref + " - " + getmobile)
                            smsdata = {"tpno": getmobile, "msg": msg}
                        ...
                        logger.info("Exception : %s" % ref + " - " + str(e))
                        return {'result': 'error', 'description': str(e), 'data': data}
