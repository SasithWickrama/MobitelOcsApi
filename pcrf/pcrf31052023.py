...
def getMobile(mobile):
    ...
    sql = 'SELECT C.CIRT_DISPLAYNAME,C.CIRT_SERT_ABBREVIATION,C.CIRT_STATUS ,SA.SATT_ATTRIBUTE_NAME,SA.SATT_DEFAULTVALUE' \
          ' FROM CIRCUITS C, SERVICES_ATTRIBUTES SA WHERE CIRT_DISPLAYNAME = :mobile AND CIRT_SERV_ID = SATT_SERV_ID ' \
...
class Pcrf:
    ...
    def Recharge(data,ref):
        ...
        if rcode == '0':
            ...
            for i in resmsg['QueryCustomerInfoResult']['Subscriber']['LifeCycleDetail']['LifeCycleStatus']:
                logger.info("OCS Response : %s" % ref + " - " + str(i['StatusIndex']))
                if i['StatusIndex'] == curIndex:
                    timestamp = int(i['StatusExpireTime'])
                    datetime_obj = datetime.strptime(i['StatusExpireTime'], "%Y%m%d%H%M%S")
                    ...
                    try:
                        ...
                        logger.info("OCS Response : %s" % ref + " - " + str(rechargeAmount))
