...
loggeracs = Logger.getLogger('acsProv', 'logs/acsProv')
...
######MYSLT_App through offer activation######
...
class acsProvision(Resource):
    def post(self):
        ...
        try:
            ...
            phone_number = data.get("PPPUNOSS", "")
            password = data.get("PPPPWOSS", "")
            
            loggeracs.info(ref + " data :" + str(data))
            
            url = f"http://acs.slt.com.lk:8086/api/ump/v3/propertyServices/serial/{serial_number}"

            loggeracs.info(ref + " url :" + str(url))
            payload = json.dumps({
                "multiShot": True,
                "domain": "/",
                "enabled": True,
                "add": {
                    "PPPUNOSS": phone_number,
                    "PPPPasswordOSS": password
                }
            })
            loggeracs.info(ref + " payload :" + str(payload))
            headers = {
            ...
            loggeracs.info(ref + " response :" + str(response))
            ...
            loggeracs.info(ref + " exception :" + str(e))
            return {"error": str(e)}          
...
#ACS Provisioning
api.add_resource(acsProvision, '/api/slt/v1/acsprov/propertyServices/')
